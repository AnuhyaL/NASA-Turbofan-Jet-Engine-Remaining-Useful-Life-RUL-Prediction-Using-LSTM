...
if st.button("Train LSTM Model"):
    ...
    # -------- TRAIN vs TEST HANDLING --------
    if is_test_data:
        st.info(
            "Test dataset detected. Per-cycle RUL labels are not available, "
            "so RMSE and R² are not computed. Showing prediction plot only."
        )
    else:
